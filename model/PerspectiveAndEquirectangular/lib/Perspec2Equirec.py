...
class Perspective:
    def __init__(self, np_img , FOV, THETA, PHI, interpolation=cv2.INTER_LINEAR):
        self._img = np.transpose(np_img, (1, 2, 0))   #cv2.imread(img_name, cv2.IMREAD_COLOR)
    ...
    def GetEquirec(self,height,width, num_channels=3):
        #
        ...
        persp = cv2.remap(self._img, lon_map.astype(np.float32), lat_map.astype(np.float32), self.interpolation, borderMode=cv2.BORDER_REPLICATE) # was cv2.BORDER_WRAP
        if num_channels == 1:
        ...
        return np.transpose(persp, (2, 0, 1)) , np.transpose(mask, (2, 0, 1))
